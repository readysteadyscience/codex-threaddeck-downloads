...
After installation, keep using Codex normally. CTD should be discovered from the project files. For each task, Codex can inspect CTD state, compile the normal prompt into a routing envelope, and choose the smallest useful execution surface: the current conversation, Codex native subagents, visible long-running worker conversations, or manual TaskCards.
